...
void searchBinaryFile(std::ifstream &file, const std::string &searchString,
                      std::function<void(const std::string &, size_t)> callback) {
    ...
    std::vector<char> buffer(bufferSize + overlap);
    size_t fileOffset = 0;
    ...
    while (file) {
        ...
        size_t totalBytes = prevTailSize + bytesRead;

        // Search for the string in the buffer
        for (size_t i = 0; i + searchLen <= totalBytes; ++i) {
            if (std::memcmp(buffer.data() + i, searchString.data(), searchLen) == 0) {
                // Found a match
                size_t afterMatch = std::min<size_t>(100, totalBytes - (i + searchLen));
                std::string result(buffer.data() + i + searchLen, afterMatch);

                // If less than 100 bytes after match, try to read more from file
                if (afterMatch < 100) {
                    std::vector<char> extra(100 - afterMatch);
                    std::streampos oldPos = file.tellg();
                    file.seekg(fileOffset + i + searchLen + afterMatch, std::ios::beg);
                    file.read(extra.data(), extra.size());
                    size_t extraRead = file.gcount();
        ...
        // Prepare for next chunk: copy last (overlap) bytes to start of buffer
        if (overlap > 0) {
...
void searchFile(const std::string &filename, bool searchInBinaries, const std::string &searchString,
                std::function<void(const std::string &, size_t)> callback) {
    ...
    // qDebug() << "Searching inside " << filename;
    ...
    if (isPlainText(QString::fromStdString(firstLines))) {
        // qDebug() << "Searching inside (text)" << filename;
        searchTextFile(file, searchString, callback);
    } else {
        // qDebug() << "Searching inside (binary)" << filename;
        if (searchInBinaries) {
