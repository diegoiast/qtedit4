...
void HistoryLineEdit::setHistoryModel(SharedHistoryModel *m)
{
    ...
    if (historyModel){    
        ...
        connect(historyModel, &SharedHistoryModel::historyUpdated, this, &HistoryLineEdit::updateCompleter);
    } else {
        if (completer) {
...
{
    qDebug() << "updating history with " << history.size() << "items, current text = " << text() ;
